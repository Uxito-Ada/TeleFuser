...
`examples/lingbot/lingbot_world_fast_image_to_video_h100.py` 提供 LingBot-World-Fast 的双向流式服务。该服务使用 WebRTC RTP 输出生成视频，通过 DataChannel 接收 prompt 和方向控制消息。当前 demo 页面不采集浏览器摄像头和麦克风；LingBot 当前仅输出视频，没有音频输出。
...
连接前在浏览器中选择初始图片。浏览器会随 WebRTC offer 直接发送图片，因此图片不需要存在于远端主机。
生成参数和默认相机内参来自流式服务配置。demo 默认开启代理，浏览器只需访问 demo 端口；
`/v1/stream/webrtc/*` 请求会由 demo 进程转发到 `--server-url`。
...
服务示例在 `examples/lingbot/lingbot_world_fast_image_to_video_h100.py` 中设置
