...
服务示例当前在 `examples/lingbot/stream_lingbot_world_fast.py` 中把
`control_type` 固定为 `"cam"`。如果要使用 action-control 权重，除了给 demo
传 `--control-mode act`，还需要修改该脚本里的 `PPL_CONFIG["control_type"]`。
...
```bash
# 在 examples/lingbot/stream_lingbot_world_fast.py 中：
PPL_CONFIG["max_area"] = 99840  # 约 416x240
```
...
| 参数 | 影响 |
|------|------|
| `PPL_CONFIG["max_area"]` | 降低输出面积，显著降低显存占用 |
