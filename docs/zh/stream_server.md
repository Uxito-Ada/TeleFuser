...
server-push 服务当前产出 `frames_b64` JPEG 数据。WebRTC 层将其解码为 `av.VideoFrame` 对象，并以目标帧率通过
RTP 流式传输到浏览器。双向服务应产出进程内 `frames`（PIL 图像或 `av.VideoFrame`），避免 JPEG/base64 往返转换。
...
客户端                          服务端
  ...
  │                               │
  │  {"type": "stop"}             │
  │  或 DELETE /v1/stream/webrtc/{session_id}
  │──────────────────────────────►│  close_session + 清理
```

客户端**必须**在生成 SDP offer 之前创建名为 `"telefuser"` 的 DataChannel。服务端复用该通道接收控制输入并发送元数据输出。视频和音频通过 RTP 媒体轨道双向传输。

输出视频默认优先使用 H.264，目标码率为 8 Mbps。可通过 `TELEFUSER_WEBRTC_VIDEO_CODEC`（`H264` 或 `VP8`）和 `TELEFUSER_WEBRTC_VIDEO_BITRATE`（单位：bit/s）覆盖。

**ChunkRouter** 是服务端的分发适配器，精确消费管线的 `pull_chunks()` 生成器一次，并进行分发：

- `frames` → 原始 PIL 或 `av.VideoFrame` → 直接推送到 `FrameGeneratorTrack`（首选，无中间有损压缩）
- `frames_b64` → 解码 JPEG → `av.VideoFrame` → 推送到 `FrameGeneratorTrack`（兼容回退）
...
server-push 数据块使用 `frames_b64`；双向数据块应使用 `frames`。`frames` 不会复制到 DataChannel：它会在服务端
转换为 RTP 视频，只有可序列化元数据通过 DataChannel 发送。

| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `type` | `str` | 是 | 固定为 `"chunk"` |
| `index` | `int` | 是 | 数据块序号 |
| `frames` | `list[PIL.Image.Image \| av.VideoFrame]` | 双向模式 | 首选的进程内视频帧，只通过 RTP 发送 |
| `frames_b64` | `list[str]` | server-push | Base64 编码的 JPEG 帧；双向模式兼容回退 |
| `fps` | `int` | 是 | 目标帧率 |
...
| `prompt` | `str` | 否 | 输入提示词 |
| `fps` | `int` | 否 | 目标视频帧率；默认使用 service 的 `default_fps`，无法获取时回退到 24 |
...
`examples/stream_server/webrtc_bidirectional_demo.py` —— LingBot-World v2 双向 WebRTC 客户端，提供：

- 通过 DataChannel 发送提示词和方向控制
- 在浏览器中选择不超过 10 MiB 的初始图片
- 服务端输出视频、运行时遥测和 DataChannel 消息日志

```bash
python examples/stream_server/webrtc_bidirectional_demo.py --server-url http://localhost:8088
```

### LingBot-World v2 流式生成

`examples/lingbot/lingbot_world_v2_image_to_video_h100.py` 提供 LingBot-World v2 的双向流式服务。该服务使用
WebRTC RTP 输出生成视频，通过 DataChannel 接收 prompt 和方向控制消息。它向 `ChunkRouter` 产出原生 PIL 帧；生成帧
不会放入 `frames_b64` payload。当前 demo 页面不采集浏览器摄像头和麦克风；LingBot 当前仅输出视频，没有音频输出。

LingBot 离线与服务路径共用基于 actor 的流式调度器。有界 edge、session 生命周期、指标和 GPU 卡位规则详见[流式 Pipeline 调度器](stream_scheduler.md)。设备重叠不会隐式创建 resource group：VAE encode、DiT 和 VAE decode 即使位于同一张 GPU 也可以并发运行。

#### 模型文件

LingBot-World v2 需要两类权重：
...
| v2 fast 模型子目录 | `/storage/model_zoo/lingbot/lingbot-world-v2-14b-causal-fast/transformers` | LingBot-World v2 causal-fast DiT 权重 |
...
连接前可以在浏览器中选择初始图片。浏览器会随 WebRTC offer 直接发送图片，因此图片不需要存在于远端主机；
未选择时，demo 使用服务端默认图片。生成参数来自流式服务配置。未显式传入内参时，LingBot 会根据所选图片
生成中心对齐的相机内参：`fx = fy = image_width`、`cx = image_width / 2`、`cy = image_height / 2`。
demo 默认开启代理，浏览器只需访问 demo 端口；`/v1/stream/webrtc/*` 请求会由 demo 进程转发到
`--server-url`。
...
每个输出 chunk 都携带提交给模型的不可变 `applied_controls` 快照。画面中的 `MOVE`/`ROTATE` HUD 和
对应的 DataChannel 状态均使用这份快照，而不是读取当前浏览器按键，因此 HUD 始终与该画面实际采用的平移和旋转一致。
...
service 默认开启 `Control HUD`。使用方向控制的输出 chunk 会在左下角显示 `MOVE` 面板，在右下角显示
`ROTATE` 面板，用于确认控制链路生效。当前 demo 页面没有 HUD 开关；需要在 session 配置中设置
`show_control_hud=false` 才能关闭。

常用控制强度参数：

| Session 配置字段 | 默认值 | 说明 |
|------------------|--------|------|
| `control_move_step` | `0.10` | 每个视频帧的前进/后退位移 |
| `control_lateral_step` | `0.10` | 每个视频帧的横向位移 |
| `control_translation_scale` | `3.0` | 相对平移按 chunk 归一化后应用的乘数 |
| `control_yaw_step_degrees` | `0.5` | 每个视频帧的偏航角度 |
| `control_pitch_step_degrees` | `0.5` | 每个视频帧的俯仰角度 |
| `control_pitch_limit_degrees` | `85.0` | 绝对俯仰角限制 |
| `show_control_hud` | `true` | 是否在受控 chunk 上叠加方向 HUD |

这些字段应放在 WebRTC offer 的 `config` 对象或 service 的 `default_session_config` 中，不是
`webrtc_bidirectional_demo.py` 或 `telefuser stream-serve` 的命令行参数。

#### 相机控制模式

LingBot-World v2 causal-fast 只支持 `cam` 控制。服务端会把移动和旋转按键转换为相机位姿，再根据位姿和内参
构建六通道 Plücker camera-control tensor。该 v2 streaming 示例不支持 `act` 控制，客户端也不能覆盖
`control_mode`。

#### 帧数与控制输入契约

LingBot 只接受完整 latent chunk。`frame_num` 必须为 `4n + 1`，并且得到的 latent frame 数必须能被
`chunk_size` 整除。v2 示例使用 `chunk_size=4`，合法输出帧数为 13、29、45、61、77、… 。
`PPL_CONFIG["frame_num"]=77` 是离线运行默认值。若 WebRTC offer 未传入 `frame_num`，stream service 会在
`max_duration_seconds` 范围内选择最长的合法长度；应在 offer 的 `config` 中显式设置
`max_duration_seconds` 或 `frame_num` 来控制浏览器 session。非法值会被拒绝。离线 control 文件必须提供覆盖
...
worker 之间分摊 KV heads，因此更多 GPU 可以支持更长 session，但不会完全线性扩展。旧 fast 模型的显存数据
不适用于 v2 causal-fast 示例；应先使用显式的较短 `frame_num` 验证目标 GPU 拓扑，再逐步增加长度。
...
| Session `frame_num` | 降低总生成帧数和 latent chunk 数 |
| Session `chunk_size` | 影响每次生成的 latent chunk 大小 |
...
| `TELEFUSER_WEBRTC_VIDEO_CODEC` | `H264` | 首选输出视频编码（`H264` 或 `VP8`） |
| `TELEFUSER_WEBRTC_VIDEO_BITRATE` | `8000000` | 输出视频目标码率，单位为 bit/s |
| `TELEFUSER_WEBRTC_DATA_CHANNEL_TIMEOUT_SECONDS` | `10` | 双向 WebRTC 必需 DataChannel 的打开超时（秒） |
| `TELEFUSER_WEBRTC_DISCONNECTED_GRACE_SECONDS` | `5` | WebRTC 短暂断连的回收宽限期（秒） |
...
只有管线输出 `audio_b64` 时浏览器才会有声音。`stream_video_replay.py` 可以包含音频；LingBot-World v2 当前只输出视频帧，不生成音频，因此 demo 中没有静音/取消静音按钮。
...
LingBot-World v2 的显存主要由 DiT 权重、VAE/text encoder 和每个 session 的 runtime/KV cache 占用。如果出现 CUDA OOM：
...
3. 降低 `PPL_CONFIG["resolution"]` 或 session 配置中的 `frame_num`
