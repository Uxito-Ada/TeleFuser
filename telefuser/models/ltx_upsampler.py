...
LTX23_SPATIAL_UPSAMPLER_CONFIG = {
    "in_channels": 128,
    "mid_channels": 1024,
...
def get_ltx23_spatial_upsampler_config() -> dict:
    return deepcopy(LTX23_SPATIAL_UPSAMPLER_CONFIG)
...
from .ltx_video_vae import VideoEncoder


class LatentUpsampler(BaseModel):
    """
...
class LTXSpatialUpsampler(LatentUpsampler):
    """Registered LTX 2.3 spatial upsampler."""

    def __init__(self) -> None:
        super().__init__(**get_ltx23_spatial_upsampler_config())

    @staticmethod
    def state_dict_converter():
        return LTXSpatialUpsamplerStateDictConverter()


class LTXSpatialUpsamplerStateDictConverter:
    """State dict converter for the standalone LTX spatial upsampler checkpoint."""
    ...
    def from_diffusers(self, state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        raise NotImplementedError("LTXSpatialUpsampler only supports the civitai-style single-file checkpoint.")


__all__ = [
    "LTXSpatialUpsampler",
    "LTXSpatialUpsamplerStateDictConverter",
    "LatentUpsampler",
