"""LTX audio decoder + vocoder (vendored).

TeleFuser's LTX2.3 pipeline generates audio in a latent mel representation (shape (B, 8, T, 16)).
To write videos with audio without relying on the external `ltx_core` package, this module vendors the
minimal decoder + vocoder implementation and config builders that match LTX checkpoints.
"""
...
class PixelNorm(nn.Module):
    """Per-location RMS normalization."""
...
class CausalConv2d(nn.Module):
    """A causal 2D convolution wrapper used by LTX audio VAE."""
...
class ResBlock1(nn.Module):
    """1D ResBlock used by the vocoder."""
...
class ResnetBlock(nn.Module):
    """2D ResNet block used by the audio decoder."""
...
class AudioPatchifier:
    """Patchifier tailored for spectrogram/audio latents (used for normalization only)."""
...
class PerChannelStatistics(nn.Module):
    """Per-channel statistics for denormalizing the latent representation."""
...
def build_mid_block(  # noqa: PLR0913
    channels: int,
    temb_channels: int,
    dropout: float,
    norm_type: NormType,
...
class AudioDecoder(nn.Module):
    """Decoder that reconstructs audio spectrograms from latent features."""
    ...
    def _denormalize_latents(self, sample: torch.Tensor) -> tuple[torch.Tensor, AudioLatentShape]:
        ...
        target_frames = latent_shape.frames * 4
        if self.causality_axis != CausalityAxis.NONE:
            target_frames = max(target_frames - (4 - 1), 1)
...
class Vocoder(nn.Module):
    def __init__(  # noqa: PLR0913
        ...
        for i in range(len(upsample_rates)):
            ...
            for kernel_size, dilations in zip(resblock_kernel_sizes, resblock_dilation_sizes, strict=True):
                if self.is_amp:
                    self.resblocks.append(AMPBlock1(ch, kernel_size, tuple(dilations), activation=activation))
                else:
                    self.resblocks.append(ResBlock1(ch, kernel_size, tuple(dilations)))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dim() == 4:
            if x.shape[1] != 2:
                raise ValueError("Input must have 2 channels for stereo")
        ...
        for i in range(self.num_upsamples):
            ...
            block_outputs = torch.stack([self.resblocks[idx](x) for idx in range(start, end)], dim=0)
...
class VocoderWithBWE(nn.Module):
    def __init__(
        ...
        # Resampler filter is not stored in checkpoint (persistent=False). Build on CPU to materialize buffers.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = self.vocoder(x)
        _, _, length_low_rate = out.shape
        output_length = length_low_rate * self.output_sampling_rate // self.input_sampling_rate

        remainder = length_low_rate % self.hop_length
        if remainder != 0:
            out = F.pad(out, (0, self.hop_length - remainder))

        mel = self._compute_mel(out)
        mel_for_bwe = mel.transpose(2, 3)
        residual = self.bwe_generator(mel_for_bwe)
        skip = self.resampler(out)
...
# -----------------------------------------------------------------------------
# Configurators (subset of ltx_core model_configurator)
# -----------------------------------------------------------------------------


class AudioDecoderConfigurator:
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> AudioDecoder:
        ...
        sample_rate = int(model_params.get("sampling_rate", 16000))
        mel_hop_length = int(stft_cfg.get("hop_length", 160))
        is_causal = bool(stft_cfg.get("causal", True))
        mel_bins = ddconfig.get("mel_bins") or mel_cfg.get("n_mel_channels") or variables_cfg.get("mel_bins")
        mel_bins = int(mel_bins) if mel_bins is not None else None

        return AudioDecoder(
            ch=int(ddconfig.get("ch", 128)),
            out_ch=int(ddconfig.get("out_ch", 2)),
...
class VocoderConfigurator:
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> Vocoder | VocoderWithBWE:
        ...
        if "bwe" not in cfg:
            return Vocoder(
                ...
                output_sampling_rate=int(cfg.get("output_sampling_rate", 24000)),
                activation=str(cfg.get("activation", "snake")),
        ...
        vocoder = Vocoder(
            ...
            upsample_initial_channel=int(vocoder_cfg.get("upsample_initial_channel", 1024)),
            resblock=str(vocoder_cfg.get("resblock", "AMP1")),
            output_sampling_rate=int(bwe_cfg.get("input_sampling_rate", 24000)),
            activation=str(vocoder_cfg.get("activation", "snakebeta")),
            use_tanh_at_final=bool(vocoder_cfg.get("use_tanh_at_final", True)),
            apply_final_activation=True,
            use_bias_at_final=bool(vocoder_cfg.get("use_bias_at_final", True)),
        )
        bwe_generator = Vocoder(
            ...
            upsample_initial_channel=int(bwe_cfg.get("upsample_initial_channel", 1024)),
            resblock=str(bwe_cfg.get("resblock", "AMP1")),
            output_sampling_rate=int(bwe_cfg.get("output_sampling_rate", 48000)),
            activation=str(bwe_cfg.get("activation", "snakebeta")),
            use_tanh_at_final=bool(bwe_cfg.get("use_tanh_at_final", True)),
            apply_final_activation=False,
            use_bias_at_final=bool(bwe_cfg.get("use_bias_at_final", True)),
        )
        mel_stft = MelSTFT(
            filter_length=int(bwe_cfg.get("n_fft", 1024)),
            hop_length=int(bwe_cfg.get("hop_length", 256)),
            win_length=int(bwe_cfg.get("n_fft", 1024)),
            n_mel_channels=int(bwe_cfg.get("num_mels", 64)),
        )
        return VocoderWithBWE(
            vocoder=vocoder,
            bwe_generator=bwe_generator,
            mel_stft=mel_stft,
            input_sampling_rate=int(bwe_cfg.get("input_sampling_rate", 24000)),
            output_sampling_rate=int(bwe_cfg.get("output_sampling_rate", 48000)),
            hop_length=int(bwe_cfg.get("hop_length", 256)),
        )
