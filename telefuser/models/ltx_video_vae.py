...
def _validate_spatial_tiling(tile_size_in_pixels: int, tile_overlap_in_pixels: int) -> None:
    """Validate spatial tiling arguments for the LTX VAE."""
    if tile_size_in_pixels < 64:
        raise ValueError(f"tile_size_in_pixels must be at least 64, got {tile_size_in_pixels}")
    if tile_size_in_pixels % 32 != 0:
        raise ValueError(f"tile_size_in_pixels must be divisible by 32, got {tile_size_in_pixels}")
    if tile_overlap_in_pixels % 32 != 0:
        raise ValueError(f"tile_overlap_in_pixels must be divisible by 32, got {tile_overlap_in_pixels}")
    if tile_overlap_in_pixels >= tile_size_in_pixels:
        raise ValueError(f"Overlap must be less than tile size, got {tile_overlap_in_pixels} and {tile_size_in_pixels}")


def _validate_temporal_tiling(tile_size_in_frames: int, tile_overlap_in_frames: int) -> None:
    """Validate temporal tiling arguments for the LTX VAE."""
    if tile_size_in_frames < 16:
        raise ValueError(f"tile_size_in_frames must be at least 16, got {tile_size_in_frames}")
    if tile_size_in_frames % 8 != 0:
        raise ValueError(f"tile_size_in_frames must be divisible by 8, got {tile_size_in_frames}")
    if tile_overlap_in_frames % 8 != 0:
        raise ValueError(f"tile_overlap_in_frames must be divisible by 8, got {tile_overlap_in_frames}")
    if tile_overlap_in_frames >= tile_size_in_frames:
        raise ValueError(f"Overlap must be less than tile size, got {tile_overlap_in_frames} and {tile_size_in_frames}")
...
def create_tiles_from_intervals_and_mappers(
    ...
    for axis_index in range(len(original_shape)):
        ...
        output_slices, masks_1d = mappers[axis_index](intervals)
...
def create_tiles(
    ...
    dimension_intervals = tuple(splitter(length) for splitter, length in zip(splitters, tensor_shape, strict=True))
    return create_tiles_from_intervals_and_mappers(tensor_shape, dimension_intervals, mappers)
...
from .ltx_dit import PixArtAlphaCombinedTimestepSizeEmbeddings
...
class VideoPixelShape(NamedTuple):
    ...
    def to_torch_shape(self, channels: int = 3) -> torch.Size:
        return torch.Size([self.batch, channels, self.frames, self.height, self.width])
...
class VideoLatentShape(NamedTuple):
    ...
    def from_torch_shape(shape: torch.Size) -> "VideoLatentShape":
        return VideoLatentShape(shape[0], shape[1], shape[2], shape[3], shape[4])
    ...
    def upscale(self, scale_factors: SpatioTemporalScaleFactors) -> VideoPixelShape:
...
class VideoEncoder(nn.Module):
    ...
    def tiled_encode(
        self,
        video: torch.Tensor,
        tile_size: tuple[int, int] = (64, 512),
        tile_stride: tuple[int, int] = (40, 448),
    ) -> torch.Tensor:
        ...
        Args:
            ...
            tile_size: Temporal/spatial tile size as `(frames, pixels)`.
            tile_stride: Temporal/spatial tile stride as `(frames, pixels)`.
        Returns:
        ...
        temporal_tile_size, spatial_tile_size = tile_size
        temporal_tile_stride, spatial_tile_stride = tile_stride
        tiles = prepare_tiles_for_encoding(
            video,
            spatial_tile_size_in_pixels=spatial_tile_size,
            spatial_tile_overlap_in_pixels=spatial_tile_size - spatial_tile_stride,
            temporal_tile_size_in_frames=temporal_tile_size,
            temporal_tile_overlap_in_frames=temporal_tile_size - temporal_tile_stride,
        )
...
def prepare_tiles_for_encoding(
    video: torch.Tensor,
    spatial_tile_size_in_pixels: int | None = None,
    spatial_tile_overlap_in_pixels: int = 0,
    temporal_tile_size_in_frames: int | None = None,
    temporal_tile_overlap_in_frames: int = 0,
) -> List[Tile]:
    """Prepare tiles for VAE encoding.
    Args:
        video: Input video tensor (B, 3, F, H, W) in range [-1, 1]
        spatial_tile_size_in_pixels: Spatial tile size in pixels.
        spatial_tile_overlap_in_pixels: Spatial overlap in pixels.
        temporal_tile_size_in_frames: Temporal tile size in frames.
        temporal_tile_overlap_in_frames: Temporal overlap in frames.
    Returns:
    ...
    if spatial_tile_size_in_pixels is not None:
        _validate_spatial_tiling(spatial_tile_size_in_pixels, spatial_tile_overlap_in_pixels)
        tile_size_px = spatial_tile_size_in_pixels
        overlap_px = spatial_tile_overlap_in_pixels
    ...
    if temporal_tile_size_in_frames is not None:
        _validate_temporal_tiling(temporal_tile_size_in_frames, temporal_tile_overlap_in_frames)
        tile_size_frames = temporal_tile_size_in_frames
        overlap_frames = temporal_tile_overlap_in_frames
...
class VideoDecoder(nn.Module):
    ...
    def _prepare_tiles(
        ...
        temporal_tile_size, spatial_tile_size = tile_size
        temporal_tile_stride, spatial_tile_stride = tile_stride
        spatial_overlap = spatial_tile_size - spatial_tile_stride
        temporal_overlap = temporal_tile_size - temporal_tile_stride

        _validate_spatial_tiling(spatial_tile_size, spatial_overlap)
        _validate_temporal_tiling(temporal_tile_size, temporal_overlap)
        ...
        if spatial_tile_size is not None:
            ...
            def enable_on_axis(axis_idx: int, factor: int) -> None:
                size = spatial_tile_size // factor
                overlap = spatial_overlap // factor
        ...
        if temporal_tile_size is not None:
            tile_size_in_latents = temporal_tile_size // self.video_downscale_factors.time
            overlap_in_latents = temporal_overlap // self.video_downscale_factors.time
            splitters[2] = split_temporal_latents(tile_size_in_latents, overlap_in_latents)
    ...
    def tiled_decode(
        ...
        tiles = self._prepare_tiles(latent, tile_size=tile_size, tile_stride=tile_stride)
...
def decode_video(
    ...
    Args:
        ...
        tiled: Whether to enable tiled decoding.
        tile_size: Temporal/spatial tile size as `(frames, pixels)`.
        tile_stride: Temporal/spatial tile stride as `(frames, pixels)`.
        generator: Optional random generator for deterministic decoding.
    Yields:
        Decoded chunk [b, c, f, h, w] in the decoder output range.
    """

    if tiled:
        for frames in video_decoder.tiled_decode(
            latent, tile_size=tile_size, tile_stride=tile_stride, generator=generator
        ):
            yield frames
    else:
...
def get_video_chunks_number(
    ...
    Args:
        ...
        tiled: Whether tiled decoding is enabled.
        tile_size: Temporal/spatial tile size as `(frames, pixels)`.
        tile_stride: Temporal/spatial tile stride as `(frames, pixels)`.
    Returns:
...
class VideoEncoderConfigurator:
    """Thin config adapter for constructing `VideoEncoder`."""

    @classmethod
    def from_config(cls: type[VideoEncoder], config: dict) -> VideoEncoder:
        return VideoEncoder(**_ltx_video_vae_encoder_kwargs_from_config(config))


class VideoDecoderConfigurator:
    """Thin config adapter for constructing `VideoDecoder`."""

    @classmethod
    def from_config(cls: type[VideoDecoder], config: dict) -> VideoDecoder:
        return VideoDecoder(**_ltx_video_vae_decoder_kwargs_from_config(config))


def _ltx_video_vae_config_section(config: dict) -> dict:
    return config.get("vae", config)


def _ltx_video_vae_encoder_kwargs_from_config(config: dict) -> dict:
    config = _ltx_video_vae_config_section(config)
    return {
        ...
        "latent_log_var": LogVarianceType(config.get("latent_log_var", "uniform")),
        "encoder_spatial_padding_mode": PaddingModeType(config.get("spatial_padding_mode", "zeros")),
    }


def _ltx_video_vae_decoder_kwargs_from_config(config: dict) -> dict:
    config = _ltx_video_vae_config_section(config)
    return {
        "convolution_dimensions": config.get("dims", 3),
        ...
        "decoder_spatial_padding_mode": PaddingModeType(config.get("spatial_padding_mode", "zeros")),
        "base_channels": config.get("decoder_base_channels", 128),
    }


def _ltx_video_vae_kwargs_from_config(config: dict) -> dict:
    config = _ltx_video_vae_config_section(config)
    return {
        "convolution_dimensions": config.get("dims", 3),
        "in_channels": config.get("in_channels", 3),
        ...
        "encoder_blocks": tuple(config.get("encoder_blocks", DEFAULT_LTX23_ENCODER_BLOCKS)),
        "decoder_blocks": tuple(config.get("decoder_blocks", DEFAULT_LTX23_DECODER_BLOCKS)),
        "patch_size": config.get("patch_size", 4),
        "norm_layer": NormLayerType(config.get("norm_layer", "pixel_norm")),
        "latent_log_var": LogVarianceType(config.get("latent_log_var", "uniform")),
        "causal_decoder": config.get("causal_decoder", False),
        "timestep_conditioning": config.get("timestep_conditioning", False),
        "spatial_padding_mode": PaddingModeType(config.get("spatial_padding_mode", "zeros")),
        "decoder_base_channels": config.get("decoder_base_channels", 128),
    }


class LTXVideoVAE(BaseModel):
    """Registered LTX 2.3 VAE wrapper with shared encoder/decoder checkpoint loading."""
    ...
    def __init__(
        ...
        encoder_blocks: tuple[tuple[str, int | dict], ...] = DEFAULT_LTX23_ENCODER_BLOCKS,
        decoder_blocks: tuple[tuple[str, int | dict], ...] = DEFAULT_LTX23_DECODER_BLOCKS,
        patch_size: int = 4,
        norm_layer: NormLayerType = NormLayerType.PIXEL_NORM,
        latent_log_var: LogVarianceType = LogVarianceType.UNIFORM,
    ...
    @classmethod
    def from_config(cls, config: dict) -> LTXVideoVAE:
        """Build the registered LTX VAE from a config dictionary."""
        return cls(**_ltx_video_vae_kwargs_from_config(config))

    @property
    def per_channel_statistics(self) -> nn.Module:
        return self.encoder.per_channel_statistics
    ...
    def encode(
        ...
        return self.encoder.tiled_encode(video, tile_size=tile_size, tile_stride=tile_stride)
    ...
    def decode(
        ...
        if not tiled:
            yield self.decoder(latent, timestep=timestep, generator=generator)
            return
        yield from self.decoder.tiled_decode(
            latent,
            tile_size=tile_size,
    ...
    def enable_sequential_cpu_offload(
        ...
        """Enable sequential CPU offload for the encoder and decoder stacks."""
        ...
        if torch_dtype is None:
            torch_dtype = next(iter(self.parameters())).dtype
...
class LTXVideoVAEStateDictConverter:
    """Convert the monolithic LTX checkpoint into the registered VAE wrapper layout."""

    _ENCODER_PREFIX = "vae.encoder."
    _DECODER_PREFIX = "vae.decoder."
    _STATS_PREFIX = "vae.per_channel_statistics."
    ...
    def from_official(self, state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        for key, value in state_dict.items():
            if key.startswith(self._ENCODER_PREFIX):
                converted_state_dict[f"encoder.{key.removeprefix(self._ENCODER_PREFIX)}"] = value
            elif key.startswith(self._DECODER_PREFIX):
                converted_state_dict[f"decoder.{key.removeprefix(self._DECODER_PREFIX)}"] = value
            elif key.startswith(self._STATS_PREFIX):
                suffix = key.removeprefix(self._STATS_PREFIX)
                converted_state_dict[f"encoder.per_channel_statistics.{suffix}"] = value
                converted_state_dict[f"decoder.per_channel_statistics.{suffix}"] = value
        return converted_state_dict

    def from_diffusers(self, state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        raise NotImplementedError("LTXVideoVAE only supports the civitai-style single-file checkpoint.")


LTXVideoDecoder = VideoDecoder
LTXVideoEncoder = VideoEncoder

__all__ = [
    "LTXVideoVAE",
    "LTXVideoVAEStateDictConverter",
    "LTXVideoDecoder",
    "LTXVideoEncoder",
    "LogVarianceType",
    "NormLayerType",
