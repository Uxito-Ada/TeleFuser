...
def get_ltx23_dev_transformer_config() -> dict:
    return deepcopy(LTX23_DEV_TRANSFORMER_CONFIG)
...
def generate_freqs(
    ...
    if use_middle_indices_grid:
        if len(indices_grid.shape) != 4:
            raise ValueError(
                "use_middle_indices_grid expects indices_grid with shape (B, D, T, bounds), got "
                f"{tuple(indices_grid.shape)}."
            )
        # Video positions include [start, end] bounds for each token; audio positions may only provide a single
        # coordinate (equivalent to start) which is sufficient when patch size is 1.
        if indices_grid.shape[-1] == 2:
            indices_grid_start, indices_grid_end = indices_grid[..., 0], indices_grid[..., 1]
            indices_grid = (indices_grid_start + indices_grid_end) / 2.0
        elif indices_grid.shape[-1] == 1:
            indices_grid = indices_grid[..., 0]
        else:
            raise ValueError(
                "use_middle_indices_grid expects indices_grid bounds dimension to be 1 or 2, got "
                f"{indices_grid.shape[-1]} for shape {tuple(indices_grid.shape)}."
            )
...
class Attention(torch.nn.Module):
    """Multi-head attention that delegates computation to `telefuser.ops.attention`.

    The attention backend is configured globally via `Attention.attention_config`, which is intended to be set by
    the runtime (e.g. `ModelRuntimeConfig.attention_config`) like other TeleFuser models.
    """

    attention_config: AttentionConfig = AttentionConfig.dense_attention(AttnImplType.TORCH_SDPA)
...
class Modality:
    ...
    Attributes:
        ...
        attention_mask: Optional 2-D self-attention mask, shape ``(B, T, T)``.
            ...
            conditioning strength wrappers.
    """
...
def build_ltx_model_from_config(config: dict) -> LTXModel:
    """Build the registered LTX transformer from a config dict.

    This mirrors the style used by other model definitions (e.g. WanVideo) by
    avoiding a dedicated `*Configurator` class and keeping the construction
    logic as a simple module-level helper.
    """
    caption_projection, audio_caption_projection = _build_caption_projections(config, is_av=True)
    transformer_config = config.get("transformer", {})
    check_config_value(transformer_config, "dropout", 0.0)
    check_config_value(transformer_config, "attention_bias", True)
    check_config_value(transformer_config, "num_vector_embeds", None)
    check_config_value(transformer_config, "activation_fn", "gelu-approximate")
    check_config_value(transformer_config, "num_embeds_ada_norm", 1000)
    check_config_value(transformer_config, "use_linear_projection", False)
    check_config_value(transformer_config, "only_cross_attention", False)
    check_config_value(transformer_config, "cross_attention_norm", True)
    check_config_value(transformer_config, "double_self_attention", False)
    check_config_value(transformer_config, "upcast_attention", False)
    check_config_value(transformer_config, "standardization_norm", "rms_norm")
    check_config_value(transformer_config, "norm_elementwise_affine", False)
    check_config_value(transformer_config, "qk_norm", "rms_norm")
    check_config_value(transformer_config, "positional_embedding_type", "rope")
    check_config_value(transformer_config, "use_audio_video_cross_attention", True)
    check_config_value(transformer_config, "share_ff", False)
    check_config_value(transformer_config, "av_cross_ada_norm", True)
    check_config_value(transformer_config, "use_middle_indices_grid", True)
    return LTXModel(
        model_type=LTXModelType.AudioVideo,
        num_attention_heads=transformer_config.get("num_attention_heads", 32),
        attention_head_dim=transformer_config.get("attention_head_dim", 128),
        in_channels=transformer_config.get("in_channels", 128),
        out_channels=transformer_config.get("out_channels", 128),
        num_layers=transformer_config.get("num_layers", 48),
        cross_attention_dim=transformer_config.get("cross_attention_dim", 4096),
        norm_eps=transformer_config.get("norm_eps", 1e-06),
        positional_embedding_theta=transformer_config.get("positional_embedding_theta", 10000.0),
        positional_embedding_max_pos=transformer_config.get("positional_embedding_max_pos", [20, 2048, 2048]),
        timestep_scale_multiplier=transformer_config.get("timestep_scale_multiplier", 1000),
        use_middle_indices_grid=transformer_config.get("use_middle_indices_grid", True),
        audio_num_attention_heads=transformer_config.get("audio_num_attention_heads", 32),
        audio_attention_head_dim=transformer_config.get("audio_attention_head_dim", 64),
        audio_in_channels=transformer_config.get("audio_in_channels", 128),
        audio_out_channels=transformer_config.get("audio_out_channels", 128),
        audio_cross_attention_dim=transformer_config.get("audio_cross_attention_dim", 2048),
        audio_positional_embedding_max_pos=transformer_config.get("audio_positional_embedding_max_pos", [20]),
        av_ca_timestep_scale_multiplier=transformer_config.get("av_ca_timestep_scale_multiplier", 1),
...
class LTXVideoTransformer(BaseModel):
    """Registered LTX 2.3 dev transformer wrapped as an x0 denoiser."""

    def __init__(self) -> None:
        super().__init__()
        self.velocity_model = build_ltx_model_from_config(get_ltx23_dev_transformer_config())
        # Avoid registering shared modules twice (e.g. aliasing `transformer_blocks`), which would
        # duplicate keys in `state_dict()` and break strict checkpoint loading.
        self.layer_name_list = ["velocity_model.transformer_blocks"]

    def set_attention_config(self, attention_config: AttentionConfig) -> None:
        """Set attention implementation configuration for all LTX attention blocks."""
        super().set_attention_config(attention_config)
        logger.info(f"ltx dit set attention config to {attention_config.attn_impl}")
        Attention.attention_config = attention_config

    @staticmethod
    def state_dict_converter():
        return LTXVideoTransformerStateDictConverter()
...
class LTXVideoTransformerStateDictConverter:
    """Convert the shared LTX checkpoint into the registered transformer layout."""

    _MODEL_PREFIX = "model.diffusion_model."
    _EMBEDDINGS_CONNECTOR_PREFIXES = (
        # Keep both singular/plural variants for robustness across checkpoint exports.
        "model.diffusion_model.video_embeddings_connector",
        "model.diffusion_model.audio_embeddings_connector",
        "model.diffusion_model.video_embedding_connector",
        "model.diffusion_model.audio_embedding_connector",
    )

    def from_official(self, state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        converted_state_dict: dict[str, torch.Tensor] = {}
        for key, value in state_dict.items():
            if key.startswith(self._EMBEDDINGS_CONNECTOR_PREFIXES):
                continue
            if key.startswith(self._MODEL_PREFIX):
                converted_state_dict[f"velocity_model.{key.removeprefix(self._MODEL_PREFIX)}"] = value
        return converted_state_dict

    def from_diffusers(self, state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        raise NotImplementedError("LTXVideoTransformer only supports the civitai-style single-file checkpoint.")
...
__all__ = [
    ...
    "LTXModel",
    "LTXVideoTransformer",
    "LTXVideoTransformerStateDictConverter",
    "Modality",
    "Perturbation",
    "PerturbationConfig",
    "PerturbationType",
