...
class SelfAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, dropout: float = 0.1, eps: float = 1e-5):
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, post_norm: bool, dropout: float = 0.1, eps: float = 1e-5):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class XLMRoberta(nn.Module):
    """XLMRoberta model without pooler and LM head."""
...
def xlm_roberta_large(pretrained: bool = False, return_tokenizer: bool = False, device: str = "cpu", **kwargs):
