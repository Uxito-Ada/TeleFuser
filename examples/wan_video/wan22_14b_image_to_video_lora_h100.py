...
PPL_CONFIG = dict(
    ...
    target_fps=16,
    dit_high_lora_configs=[
        LoraConfig(
            "/nvfile-heatstorage/model_zoo/release/video/wan_cfg_step_distill/wan2.2_i2v_A14b_high_noise_lora_rank64_lightx2v_4step_1022.safetensors",
            1.0,
        )
    ],
...
def get_pipeline(parallelism=1, model_root=PPL_CONFIG["model_root"]):
    ...
    # dit high
    module_manager.load_model(
        f"{model_root}/dit_high_noise_model_bfloat16_5d6fd.safetensors",
        torch_dtype=torch.bfloat16,
    )
    # dit low
    module_manager.load_model(
        f"{model_root}/dit_low_noise_model_bfloat16_c55d6.safetensors",
        torch_dtype=torch.bfloat16,
    )
    ...
    pipe_config.dit_high_config.lora_configs = PPL_CONFIG["dit_high_lora_configs"]
