...
def run(
    ...
    Args:
        ...
        image (PIL.Image): Input image, resolution should match height/width parameters
    ...
    """
    width, height = get_target_image_size(image.size[0], image.size[1], resolution, 1, 1)
    video, _ = pipeline(
        prompt=prompt,
        input_image=image,
...
def run_with_file(
    ...
    **kwargs,
    ...
    video = run(pipeline, image, prompt, negative_prompt, seed, resolution)
...
def main(gpu_num, image_path, prompt, negative_prompt, resolution, seed, model_root):
