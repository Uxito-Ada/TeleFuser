...
from tf_kernel.utils import _get_cache_buf


def int8_scaled_mm(mat_a, mat_b, scales_a, scales_b, out_dtype, bias=None):
    return torch.ops.tf_kernel.int8_scaled_mm.default(
        mat_a,
        mat_b,
        scales_a,
        scales_b,
...
def fp8_blockwise_scaled_mm(mat_a, mat_b, scales_a, scales_b, out_dtype):
    return torch.ops.tf_kernel.fp8_blockwise_scaled_mm.default(
        mat_a,
        mat_b,
        scales_a,
        scales_b,
...
def fp8_scaled_mm(mat_a, mat_b, scales_a, scales_b, out_dtype, bias=None):
    return torch.ops.tf_kernel.fp8_scaled_mm.default(
        mat_a,
        mat_b,
        scales_a,
        scales_b,
...
def _bmm_fp8_internal(
    ...
    torch.ops.tf_kernel.bmm_fp8.default(
        A,
        B,
        D,
        A_scale,
...
def tf_per_token_group_quant_8bit(
    input: torch.Tensor,
    ...
    if enable_v2 is None:
        from tf.srt.utils import get_bool_env_var

        enable_v2 = get_bool_env_var("SGLang_PER_TOKEN_GROUP_QUANT_8BIT_V2")

    if enable_v2:
        return torch.ops.tf_kernel.tf_per_token_group_quant_8bit_v2.default(
            input,
            output_q,
            output_s,
            group_size,
    ...
    torch.ops.tf_kernel.tf_per_token_group_quant_8bit.default(
        input, output_q, output_s, group_size, eps, fp8_min, fp8_max, scale_ue8m0
    )


# For legacy usage
tf_per_token_group_quant_fp8 = tf_per_token_group_quant_8bit
tf_per_token_group_quant_int8 = tf_per_token_group_quant_8bit


def tf_per_tensor_quant_fp8(
    input: torch.Tensor,
    output_q: torch.Tensor,
    output_s: torch.Tensor,
    is_static: bool,
) -> None:
    torch.ops.tf_kernel.tf_per_tensor_quant_fp8.default(
        input, output_q, output_s, is_static
    )


def tf_per_token_quant_fp8(
    input: torch.Tensor,
    output_q: torch.Tensor,
    output_s: torch.Tensor,
) -> None:
    torch.ops.tf_kernel.tf_per_token_quant_fp8.default(input, output_q, output_s)
...
def cutlass_scaled_fp4_mm(
    ...
    torch.ops.tf_kernel.cutlass_scaled_fp4_mm.default(
...
def scaled_fp4_quant(
    ...
    torch.ops.tf_kernel.scaled_fp4_quant.default(
...
def scaled_fp4_grouped_quant(
    ...
    torch.ops.tf_kernel.silu_and_mul_scaled_fp4_experts_quant.default(
        output.view(l * m, k // 2),
...
def scaled_fp4_experts_quant(
    ...
    Args:
        input: The input tensor to be quantized to FP4
        expert_map: The expert map tensor (not supported, must be None)
        input_global_scale: A scalar scaling factor for the entire tensor.
    ...
    Note:
        expert_map is not supported in this version. The input_tensor must already
        be properly shuffled if needed.
    """
    assert (
        input_tensor.ndim == 2
    ...
    if expert_map is not None:
        raise NotImplementedError(
            "expert_map is not supported in this version. "
            "Please shuffle the input tensor before calling this function."
        )
    ...
    torch.ops.tf_kernel.scaled_fp4_experts_quant.default(
        output,
        output_scales,
        input_tensor,
        input_global_scale,
