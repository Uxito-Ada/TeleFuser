...
from tf_kernel import fp8_scaled_mm as tf_scaled_mm
...
def benchmark(batch_size, provider, N, K):
    ...
    # vLLM expects scalar scales, while tfkernel can handle per-token scales
    ...
    try:
        ...
        elif "tfkernel-fp8" in provider:
            a_fp8, scale_a_fp8 = tfkernel_scaled_fp8_quant(a, scale_a)
            b_fp8, scale_b_fp8 = tfkernel_scaled_fp8_quant(b, scale_b)
            b_fp8 = b_fp8.t()
            ms, min_ms, max_ms = triton.testing.do_bench_cudagraph(
                lambda: tf_scaled_mm(
...
if __name__ == "__main__":
    ...
    for K, N, model_name in KN_model_names:
        ...
        with tqdm(
            total=total_batch_sizes * total_providers, desc=f"N={N}, K={K}", unit="test"
        ) as pbar:
