...
// Include headers for torch extension
...
void run_mha_fwd_block(Flash_fwd_params& params, cudaStream_t stream, bool force_split_kernel = false) {
  FP16_SWITCH(!params.is_bf16, [&] {
    HEADDIM_SWITCH(params.d, [&] {
      BOOL_SWITCH(
          params.is_causal, Is_causal, [&] { run_mha_fwd_block_<elem_type, kHeadDim, Is_causal>(params, stream); });
    });
  });
}
...
std::vector<at::Tensor> mha_varlen_fwd_block(
    ...
    std::optional<at::Tensor>&
        row_blockmask_,  // (batch_size, num_blocksparse_heads, seqlen_m / m_block_dim, seqlen_n / n_block_dim)
    int max_seqlen_q,
  ...
  if (has_blockmask) {
    ...
    CHECK_SHAPE(
        ...
        seqlen_q_rounded / m_block_dim,
        seqlen_k_rounded / n_block_dim);
  }
  if (has_streaming_info) {
...
void run_mha_bwd_block(Flash_bwd_params& params, cudaStream_t stream) {
  FP16_SWITCH(!params.is_bf16, [&] {
    HEADDIM_SWITCH(params.d, [&] {
      BOOL_SWITCH(
          params.is_causal, Is_causal, [&] { run_mha_bwd_block_<elem_type, kHeadDim, Is_causal>(params, stream); });
    });
  });
}
...
std::vector<at::Tensor> mha_varlen_bwd_block(
    ...
    std::optional<at::Tensor>&
        col_blockmask_,  // (batch_size, num_blocksparse_heads, seqlen_n / n_block_dim, seqlen_m / m_block_dim)
...
// Interface functions exposed to tf_kernel
namespace tf_kernel {
...
std::vector<at::Tensor> block_sparse_attn_fwd(
    ...
    bool return_softmax,
    std::optional<at::Generator> gen_) {
  return FLASH_NAMESPACE::mha_varlen_fwd_block(
      q,
      k,
      v,
      cu_seqlens_q,
      ...
      static_cast<int>(m_block_dim),
      static_cast<int>(n_block_dim),
      exact_streaming,
      return_softmax,
      gen_);
}
...
std::vector<at::Tensor> block_sparse_attn_bwd(
    ...
    std::optional<at::Generator> gen_,
    std::optional<at::Tensor> rng_state) {
  return FLASH_NAMESPACE::mha_varlen_bwd_block(
      dout,
      q,
      k,
      v,
