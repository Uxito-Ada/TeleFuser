...
template <bool Is_first, bool Check_inf = false, typename Tensor0, typename Tensor1, typename Tensor2>
inline __device__ void softmax_rescale_o_block(
    Tensor0& scores,
    Tensor1& scores_max,
    Tensor1& scores_sum,
    Tensor2& acc_o,
    float softmax_scale_log2,
    bool Is_blocksparse_skip) {
  if (Is_first) {
    FLASH_NAMESPACE::template reduce_max</*zero_init=*/true>(scores, scores_max);
    FLASH_NAMESPACE::scale_apply_exp2(scores, scores_max, softmax_scale_log2);
    FLASH_NAMESPACE::reduce_sum(scores, scores_sum);
...
#pragma unroll
    for (int mi = 0; mi < size(scores_max); ++mi) {
      float scores_max_cur =
          !(Check_inf || Is_blocksparse_skip) ? scores_max(mi) : (scores_max(mi) == -INFINITY ? 0.0f : scores_max(mi));
      float scores_scale = exp2f((scores_max_prev(mi) - scores_max_cur) * softmax_scale_log2);
      scores_sum(mi) *= scores_scale;
#pragma unroll
      for (int ni = 0; ni < size<1>(acc_o_rowcol); ++ni) {
...
#pragma unroll
  for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
    ...
    masking_step == 0 ? softmax_rescale_o</*Is_first=*/true, /*Check_inf=*/Is_causal || Is_local>(
                            scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2)
                      : softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local>(
                            scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2);
    ...
    if (Return_softmax) {
      ...
      flash::apply_dropout</*encode_dropout_in_sign_bit=*/true>(
          tOrP_copy, params.p_dropout_in_uint8_t, seed, offset, block_row_idx, block_col_idx, kNWarps);
      flash::write_softmax_to_gmem(tOrP_copy, tPgP, gmem_tiled_copy_P);
      tPgP.data() = tPgP.data() + (-kBlockN);
    }
    if (Is_dropout) {
      flash::apply_dropout(tOrP, params.p_dropout_in_uint8_t, seed, offset, block_row_idx, block_col_idx, kNWarps);
    }
  ...
  for (; n_block >= n_block_min; --n_block) {
    ...
    if (Is_local && n_block * kBlockN < (m_block + 1) * kBlockM + binfo.actual_seqlen_k - binfo.actual_seqlen_q +
                                            params.window_size_right) {
...
template <
    ...
    typename Params>
inline __device__ void
compute_block_attn_1rowblock(const Params& params, const int bidb, const int bidh, const int m_block) {
  using Element = typename Kernel_traits::Element;
  using ElementAccum = typename Kernel_traits::ElementAccum;
  using index_t = typename Kernel_traits::index_t;
  ...
  __syncthreads();
  // if (threadIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
  //         printf("m_block = %d, n_block_min = %d, n_block_max = %d, empty_line = %d, params.window_size_right = %d,
  //         devidee = %d, devider = %d， kBlcokM = %d, actual_seqlen_k = %d, actual_seqlen_q = %d\n", m_block,
  //         n_block_min, n_block_max, empty_line_flag, params.window_size_right, (m_block + 1) * kBlockM +
  //         binfo.actual_seqlen_k - binfo.actual_seqlen_q + params.window_size_right, kBlockN, kBlockM,
...
#pragma unroll
  for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
    ...
    if (is_skip) {
      ...
      masking_step == 0 ? softmax_rescale_o</*Is_first=*/true, /*Check_inf=*/true>(
                              scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2)
                        : softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/true>(
                              scores, scores_max, scores_sum, acc_o, params.scale_softmax_log2);
      ...
      // if using m16n8k16 or ((2, 2, 1), MMA_M, MMA_N) if using m16n8k8.
      Tensor tOrP =
          make_tensor(rP.data(), FLASH_NAMESPACE::convert_layout_rowcol_Aregs<Kernel_traits::TiledMma>(rP.layout()));
      // int block_row_idx = m_block * (kBlockM / 16) + tidx / 32;
      // int block_col_idx = n_block * (kBlockN / 32);
      ...
      if (!Is_causal && !Is_local && !Is_exact_streaming) {
        ...
        // // Convert to ((2, 2), MMA_M, MMA_N) then take only the row indices.
        // Tensor idx_row = logical_divide(taccScS, Shape<_2>{})(make_coord(0, _), _, 0);
        // Tensor idx_rowcol = make_tensor(taccScS.data(),
        // FLASH_NAMESPACE::convert_layout_acc_rowcol(taccScS.layout()));
        ...
        FLASH_NAMESPACE::apply_mask_streaming</*HasWSLeft=*/Is_exact_streaming>(
            ...
            binfo.actual_seqlen_q,
            kNWarps * 16,
            params.streaming_info[bidh * 2 + 1],
            params.streaming_info[bidh * 2]);
      }
      ...
      if (Is_dropout) {
        FLASH_NAMESPACE::apply_dropout(
            tOrP, params.p_dropout_in_uint8_t, seed, offset, block_row_idx, block_col_idx, kNWarps);
      }
      // if (cute::thread0()) { print(tOrP); }
  ...
  //         printf("[compute_block_attn_1rowblock] out bidb = %d, bidh = %d, m_block = %d, n_block = %d， mask_val =
  ...
  for (; !is_last_block && n_block >= n_block_min; n_block = next_block_col_idx) {
    // if(bidb == 0 && bidh == 0 && m_block == 0 && threadIdx.x == 0){
    //     printf("[compute_block_attn_1rowblock] in bidb = %d, bidh = %d, m_block = %d, n_block = %d， mask_val = %d,
    //     leap = %d\n", bidb, bidh, m_block, n_block, mask_val, leap);
    // }
    ++mask_block_idx;
    mask_val = blockmask.mask_val(mask_block_idx);
    ...
    if (Is_exact_streaming && n_block * kBlockN < (m_block + 1) * kBlockM + binfo.actual_seqlen_k -
...
inline __device__ void compute_block_attn(const Params& params) {
  ...
  // FLASH_NAMESPACE::compute_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN,
  // Is_even_K, Return_softmax>(params, bidb, bidh, m_block);
  ...
  if (head_mask_type == 0) {
    ...
    FLASH_NAMESPACE::compute_block_attn_1rowblock<
        Kernel_traits,
        Is_dropout,
        Is_causal,
        Is_local,
        ...
        Return_softmax,
        false,
        false>(params, bidb, bidh, m_block);  // false for blocksparse
  } else {
    FLASH_NAMESPACE::compute_block_attn_1rowblock<
        Kernel_traits,
        Is_dropout,
        ...
        Return_softmax,
        true,
        Is_exact_streaming>(params, bidb, bidh, m_block);  // true for streaming
  }
}
