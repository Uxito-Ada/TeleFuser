      ...
      BOOL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !Is_causal, Is_local, [&] {
        BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
          BOOL_SWITCH(params.is_exact_streaming, Is_exact_streaming, [&] {
            ...
            auto kernel = &flash_fwd_block_kernel < Kernel_traits, Is_dropout, Is_causal, Is_local && !Is_causal, false,
                 IsEvenMNConst && IsEvenKConst && !Is_local && !ReturnSoftmaxConst && Kernel_traits::kHeadDim <= 128,
                 IsEvenKConst, ReturnSoftmaxConst && Is_dropout, Is_exact_streaming && Is_causal > ;
...
void run_mha_fwd_block_hdim32(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
    run_flash_fwd_block<Flash_fwd_kernel_traits<Headdim, 128, 128, 4, false, false, T>, Is_dropout, Is_causal>(
        params, stream);
  });
}
...
void run_mha_fwd_block_hdim64(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
    if constexpr (!Is_dropout) {
      ...
      run_flash_fwd_block<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T>, Is_dropout, Is_causal>(
          params, stream);
    }
  });
}
    ...
    if constexpr (!Is_dropout) {
      ...
      if (is_sm8x) {
        if constexpr (!Is_causal) {
          run_flash_fwd_block<Flash_fwd_kernel_traits<Headdim, 128, 32, 4, false, false, T>, Is_dropout, Is_causal>(
              params, stream);
        } else {
          run_flash_fwd_block<Flash_fwd_kernel_traits<Headdim, 64, 64, 4, false, false, T>, Is_dropout, Is_causal>(
              params, stream);
        }
      } else {
        run_flash_fwd_block<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T>, Is_dropout, Is_causal>(
