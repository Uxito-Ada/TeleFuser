 ...
 public:
  ...
  __device__ fwdStreaming(
    ...
    if (causal) {
      ...
      this->start_block_val =
          (cute::ceil_div(max(act_k - act_q, 0), n_block_dim) + 1 + loop_step_idx / row_factor - start_row_idx) *
          col_factor;
    } else {
  ...
  __device__ fwdBlockmask(
    ...
    blockmask_ptr = params.blockmask +
                    (batch_idx * params.num_blocksparse_heads + mask_type - 1) *
                        int(params.seqlen_q_rounded / m_block_dim) * int(params.seqlen_k_rounded / n_block_dim) +
                    int(loop_step_idx / row_factor) * int(params.seqlen_k_rounded / n_block_dim);
  };
...
struct fwdIterator<false, false> : public fwdBlockmask {
  ...
  __device__ fwdIterator(
      ...
      int n_block_min,
      int n_block_max)
      : fwdBlockmask(params, binfo, kBlockM, kBlockN, batch_idx, head_idx, loop_step_idx, n_block_min, n_block_max) {};
};
...
struct fwdIterator<true, false> : public fwdStreaming {
  ...
  __device__ fwdIterator(
      ...
      int n_block_min,
      int n_block_max)
      : fwdStreaming(params, binfo, kBlockM, kBlockN, batch_idx, head_idx, loop_step_idx, n_block_min, n_block_max) {};
};
...
struct fwdIterator<true, true> : public fwdExactStreaming {
  ...
  __device__ fwdIterator(
      ...
      int n_block_max)
      : fwdExactStreaming(
            params, binfo, kBlockM, kBlockN, batch_idx, head_idx, loop_step_idx, n_block_min, n_block_max) {};
};
...
struct bwdStreaming : public bwdIteratorBase {
 public:
  ...
  __device__ bwdStreaming(
    ...
    int sink = (this->mask_type) < 0 ? params.streaming_info[head_idx * 2]
                                     : cute::ceil_div(binfo.actual_seqlen_k, this->n_block_dim);
    ...
    if (mask_block_col <= sink) {
      ...
      if (causal) {
        ...
        int num_mask_block_in_end_row = max(
            0, cute::ceil_div(act_k - act_q + (end_mask_block_row_idx + 1) * params.m_block_dim, params.n_block_dim));
        ...
        bool in_none_causal_local =
            !causal && mask_block_col <= n_mask_block_col && mask_block_col > n_mask_block_col - local;
...
struct bwdBlockmask : public bwdIteratorBase {
 public:
  ...
  __device__ bwdBlockmask(
    ...
    blockmask_ptr = params.blockmask +
                    (batch_idx * params.num_blocksparse_heads + mask_type - 1) *
                        int(params.seqlen_k_rounded / n_block_dim) * int(params.seqlen_q_rounded / m_block_dim) +
                    int(loop_step_idx / col_factor) * int(params.seqlen_q_rounded / m_block_dim);
  };
...
struct bwdIterator<false> : public bwdBlockmask {
  ...
  __device__ bwdIterator(
      ...
      int m_block_min,
      int m_block_max)
      : bwdBlockmask(params, binfo, kBlockM, kBlockN, batch_idx, head_idx, loop_step_idx, m_block_min, m_block_max) {};
};
...
struct bwdIterator<true> : public bwdStreaming {
  ...
  __device__ bwdIterator(
      ...
      int m_block_min,
      int m_block_max)
      : bwdStreaming(params, binfo, kBlockM, kBlockN, batch_idx, head_idx, loop_step_idx, m_block_min, m_block_max) {};
};
