 ...
 *https://github.com/Dao-AILab/flash-attention/blob/main/csrc/flash_attn/src/flash_bwd_launch_template.h
 ******************************************************************************/
...
__global__ void flash_bwd_dq_dk_dv_loop_seqk_parallel_kernel(Flash_bwd_params params) {
  ...
  FLASH_NAMESPACE::
      compute_dq_dk_dv_seqk_parallel<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K>(
          params);
}
...
__global__ void flash_bwd_block_dq_dk_dv_loop_seqk_parallel_kernel(Flash_bwd_params params) {
  static_assert(!(Is_causal && Is_local));  // If Is_local is true, Is_causal should be false
  FLASH_NAMESPACE::
      compute_block_dq_dk_dv_seqk_parallel<Kernel_traits, Is_dropout, Is_causal, Is_local, Is_even_MN, Is_even_K>(
          params);
}
  ...
  BOOL_SWITCH(is_even_MN, IsEvenMNConst, [&] {
    BOOL_SWITCH(is_even_K, IsEvenKConst, [&] {
      BOOL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !params.is_causal, Is_local, [&] {
        ...
        auto kernel = &flash_bwd_block_dq_dk_dv_loop_seqk_parallel_kernel < Kernel_traits, Is_dropout, Is_causal,
             Is_local && !Is_causal, IsEvenMNConst && IsEvenKConst && !Is_local && Kernel_traits::kHeadDim <= 128,
             IsEvenKConst > ;
  ...
  BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
    if (max_smem_per_block >= 2 * ((3 * 128 + 2 * 128) * Headdim + 2 * 128 * 128)) {  // 104 KB
      if constexpr (!Is_dropout) {  // We can afford more registers to keep V in registers
        run_flash_bwd_block<
            Flash_bwd_kernel_traits<Headdim, 128, 128, 8, 4, 4, 4, true, false, T>,
            Is_dropout,
...
void run_mha_bwd_block_hdim64(Flash_bwd_params& params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
    ...
    // configure); run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, false, false, T>,
    // Is_dropout>(params, stream, configure); This is slightly faster. We want to split M more so we need fewer
    // registers to store LSE.
    if (max_smem_per_block >= 144 * 1024) {
      ...
      // stream, configure);
      run_flash_bwd_block<Flash_bwd_kernel_traits<Headdim, 64, 128, 8, 2, 4, 4, true, false, T>, Is_dropout, Is_causal>(
          params, stream);
      // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, false, false, T>, Is_dropout>(params,
      // stream, configure); run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, true, false, T>,
      // Is_dropout>(params, stream, configure);
      // } else {
      //     run_flash_bwd_seqq_parallel<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 4, false, false, T>,
      //     Is_dropout>(params, stream, configure);
      // }
    }
  });
...
void run_mha_bwd_block_hdim128(Flash_bwd_params& params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
    ...
    if (max_smem_per_block >= 144 * 1024) {
      ...
      // Is_dropout>(params, stream, configure); run_flash_bwd_seqk_parallel<Flash_bwd_kernel_traits<Headdim, 128, 128,
      // 8, 4, 4, 4, false, true, T>, Is_dropout>(params, stream, configure);
      // run_flash_bwd_seqq_parallel<Flash_bwd_kernel_traits<Headdim, 128, 128, 8, 4, 4, 4, false, false, T>,
      // Is_dropout>(params, stream, configure); run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 64, 128, 8, 2, 4, 2,
      // true, false, T>, Is_dropout>(params, stream, configure); run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128,
      // 64, 8, 4, 2, 2, false, false, T>, Is_dropout>(params, stream, configure);
      // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 2, 2, true, false, T>, Is_dropout>(params,
      // stream, configure);
    } else {
      // run_flash_bwd<Flash_bwd_kernel_traits<Headdim, 64, 64, 8, 4, 2, 2, false, false, T>, Is_dropout>(params,
      // stream, configure);
      run_flash_bwd_block<Flash_bwd_kernel_traits<Headdim, 64, 64, 8, 4, 2, 2, true, false, T>, Is_dropout, Is_causal>(
          params, stream);
    }
    ...
    // } else {
    // run_flash_bwd_seqq_parallel<Flash_bwd_kernel_traits<Headdim, 128, 64, 8, 4, 4, 4, false, false, T>>(params,
    // stream, configure);
    // }
  });
}
