...
inline __device__ void apply_mask_local(
...
template <bool HasWSLeft = true, typename Engine, typename Layout>
inline __device__ void apply_mask_streaming(
    Tensor<Engine, Layout>& tensor,
    const int col_idx_offset_,
    const int max_seqlen_k,
    const int row_idx_offset,
...
template <typename Engine, typename Layout>
inline __device__ void apply_mask_causal(
    Tensor<Engine, Layout>& tensor,
    const int col_idx_offset_,
    const int max_seqlen_k,
    const int row_idx_offset,
  ...
  apply_mask_local</*HasWSLeft=*/false>(
      tensor, col_idx_offset_, max_seqlen_k, row_idx_offset, max_seqlen_q, warp_row_stride, -1, 0);
}
