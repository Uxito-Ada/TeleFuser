 ...
 *https://github.com/Dao-AILab/flash-attention/blob/main/csrc/flash_attn/src/flash_bwd_kernel.h
 ******************************************************************************/
...
#pragma unroll
  ...
  int m_block_min =
      (!Is_causal && !Is_local)
          ...
          : std::max(
                0,
                (n_block * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k - params.window_size_right) /
                    kBlockM);
...
#pragma unroll
    ...
    if (Is_dropout) {
      ...
      Tensor scores_dropped = make_tensor(
          scores.data(), FLASH_NAMESPACE::convert_layout_rowcol_Aregs<Kernel_traits::TiledMmaSdP>(scores.layout()));
      FLASH_NAMESPACE::apply_dropout</*encode_dropout_in_sign_bit=*/true>(
          scores_dropped, params.p_dropout_in_uint8_t, seed, offset, block_row_idx, block_col_idx, AtomLayoutMS);
    }
    // Convert scores from fp32 to fp16/bf16
    Tensor rP = !Is_dropout ? FLASH_NAMESPACE::convert_type<Element>(scores)
    ...
    Tensor tPrP =
        make_tensor(rP.data(), FLASH_NAMESPACE::convert_layout_rowcol_Aregs<Kernel_traits::TiledMmaSdP>(rP.layout()));
...
template <
    ...
    typename Params>
inline __device__ void
compute_block_dq_dk_dv_1colblock(const Params& params, const int bidb, const int bidh, const int n_block) {
...
inline __device__ void compute_block_dq_dk_dv_seqk_parallel(const Params& params) {
  ...
  for (int n_block = blockIdx.x; n_block < (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN;
    ...
    if (head_mask_type > 0) {
      ...
      //     compute_block_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Is_even_MN, Is_even_K,
      //     false, false, /*Is_streaming*/false, /*Seq_parallel=*/true>(params, bidb, bidh, n_block);
    } else {
      compute_block_dq_dk_dv_1colblock<
          Kernel_traits,
          Is_dropout,
