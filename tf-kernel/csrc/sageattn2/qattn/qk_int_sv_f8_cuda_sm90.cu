...
store_async_4D(void const* dst_tma_map, T* src, int global_token_idx, int global_head_idx, int global_batch_idx) {
  ...
  asm volatile(
      ...
      " [%0, {%2, %3, %4, %5}], [%1];"
      :
      : "l"(tma_ptr), "r"(src_ptr), "n"(0), "r"(global_token_idx), "r"(global_head_idx), "r"(global_batch_idx)
      : "memory");
}
...
__global__ void qk_int8_sv_f8_attn_kernel(
  ...
  if constexpr (Q_GRAN == QuantGranularity::kPerBlock) {
    ...
    const uint32_t num_warp_block_q = gridDim.x * 4;
    q_scale_idx = batch_id * num_qo_heads * (num_warp_block_q * 8) + head_id * (num_warp_block_q * 8) + bx * (4 * 8) +
                  warp_idx * 8 + lane_id / 4;
  }
  ...
  if constexpr (K_GRAN == QuantGranularity::kPerBlock || K_GRAN == QuantGranularity::kPerWarp) {
    ...
    k_scale_idx = batch_id * (num_qo_heads / num_kv_groups) * (num_block_k * 4) +
                  (head_id / num_kv_groups) * (num_block_k * 4) + lane_id % 4;
  }
...
#pragma unroll
  ...
  DTypeOut* O_lane_ptr = O + batch_id * stride_bz_o + head_id * stride_h_o +
                         (bx * CTA_Q + warp_idx * 16 + (lane_id / 4)) * stride_seq_o + (lane_id % 4) * 2;
#pragma unroll
  for (uint32_t fq = 0; fq < num_tiles_q; fq++) {
#pragma unroll
          ...
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_type, DTypeOut, {
            ...
            if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerWarp)) {
              CHECK_SHAPE(
                  ...
                  batch_size,
                  num_qo_heads,
                  static_cast<long>(div_ceil(qo_len, CTA_Q) * (NUM_THREADS / 32)));
              ...
              CHECK_SHAPE(
                  ...
                  batch_size,
                  num_qo_heads,
                  static_cast<long>(div_ceil(qo_len, CTA_Q) * (NUM_THREADS / 32) * 8));
            ...
            CUtensorMap tma_map_Q = create_tensor_map_4D<CTA_Q, HEAD_DIM>(
                ...
                stride_h_q,
                stride_seq_q);
            CUtensorMap tma_map_K = create_tensor_map_4D<CTA_K, HEAD_DIM>(
                ...
                stride_h_k,
                stride_seq_k);
            CUtensorMap tma_map_V = create_tensor_map_4D<HEAD_DIM, CTA_K>(
            ...
            auto* kernel = qk_int8_sv_f8_attn_kernel<
                CTA_Q,
                CTA_K,
                NUM_THREADS,
                HEAD_DIM,
