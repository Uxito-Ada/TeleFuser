...
__device__ __forceinline__ float convert_to_float(T val) {
  static_assert(
      std::is_same<T, half>::value || std::is_same<T, nv_bfloat16>::value, "Only half and bfloat16 are supported");
...
__global__ void QuantInt8Kernel(
  ...
  T* mean_ptr_base =
      mean + batch_id * stride_bz_mean + head_id * stride_h_mean + thread_id % num_threads_per_token * pack_size;
...
__global__ void TransposePadPermuteKernel(
  ...
  T* input_ptr_base = input + batch_id * stride_bz_input + head_id * stride_h_input +
                      thread_base_token * stride_seq_input + thread_id % num_threads_per_token * pack_size;
  T* output_ptr_base = output + batch_id * stride_bz_output + head_id * stride_h_output + bx * CTA_SIZE +
                       thread_id % num_threads_per_cta * pack_size + thread_id / num_threads_per_cta * stride_d_output;
  ...
  uint32_t smem_load_row =
      smem_load_row_base + (smem_load_row_mod / 8) * 2 + ((smem_load_row_mod / 2) % 4) * 4 + (smem_load_row_mod % 2);
...
__global__ void MeanScaleKernel(
    T* __restrict__ input,
    int8_t* __restrict__ output,
    ...
    const uint32_t stride_h_scale) {
  static_assert(
      std::is_same<T, half>::value || std::is_same<T, __nv_bfloat16>::value, "Only half and bfloat16 are supported");
  ...
  T* input_ptr_base =
      input + batch_id * stride_bz_input + head_id * stride_h_input + d_id * stride_d_input + thread_id * pack_size;
  int8_t* output_ptr_base =
      output + batch_id * stride_bz_output + head_id * stride_h_output + d_id * stride_d_output + thread_id * pack_size;
...
void quant_per_block_int8_cuda(
    torch::Tensor input, torch::Tensor output, torch::Tensor scale, int64_t block_size, int64_t tensor_layout) {
  CHECK_CUDA(input);
  CHECK_CUDA(output);
  CHECK_CUDA(scale);
      ...
      DISPATCH_WARP_BLOCK_SIZE(warp_block_size, WARP_BLOCK_SIZE, {
        DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
          ...
          CHECK_SHAPE(
              ...
              batch_size,
              num_heads,
              (num_tokens + BLOCK_SIZE - 1) / BLOCK_SIZE * (BLOCK_SIZE / WARP_BLOCK_SIZE));

          dim3 grid((num_tokens + BLOCK_SIZE - 1) / BLOCK_SIZE * (BLOCK_SIZE / WARP_BLOCK_SIZE), num_heads, batch_size);

          constexpr int64_t num_pack_per_thread = (WARP_BLOCK_SIZE * (HEAD_DIM / 8) + 1023) / 1024;
...
void sub_mean_cuda(torch::Tensor input, torch::Tensor mean, torch::Tensor output, int64_t tensor_layout) {
  CHECK_CUDA(input);
  CHECK_CUDA(mean);
  CHECK_CUDA(output);
...
void transpose_pad_permute_cuda(torch::Tensor input, torch::Tensor output, int64_t tensor_layout) {
  CHECK_CUDA(input);
  CHECK_CUDA(output);
