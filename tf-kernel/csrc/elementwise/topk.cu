...
// per-arch value from `setup_rocm.py` via `-DTF_TOPK_DYNAMIC_SMEM_BYTES=...`.
#ifdef TF_TOPK_DYNAMIC_SMEM_BYTES
constexpr size_t kSmem = static_cast<size_t>(TF_TOPK_DYNAMIC_SMEM_BYTES);
#else
