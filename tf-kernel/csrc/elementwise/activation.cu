...
#if USE_ROCM
    tf_hip::activation::act_and_mul_kernel<c_type, silu>
...
void gelu_tanh_and_mul(at::Tensor& out, at::Tensor& input) {
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FLOAT_FP16(input.scalar_type(), c_type, [&] {
    ...
    dim3 block(std::min(d / vec_size, 1024U));
#if USE_ROCM
    tf_hip::activation::act_and_mul_kernel<c_type, gelu_tanh>
        <<<grid, block, 0, stream>>>(static_cast<c_type*>(out.data_ptr()), static_cast<c_type*>(input.data_ptr()), d);
#else
...
void gelu_and_mul(at::Tensor& out, at::Tensor& input) {
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FLOAT_FP16(input.scalar_type(), c_type, [&] {
    ...
    dim3 block(std::min(d / vec_size, 1024U));
#if USE_ROCM
    tf_hip::activation::act_and_mul_kernel<c_type, gelu>
        <<<grid, block, 0, stream>>>(static_cast<c_type*>(out.data_ptr()), static_cast<c_type*>(input.data_ptr()), d);
#else
...
void gelu_quick(at::Tensor& out, const at::Tensor& input) {
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FLOAT_FP16(input.scalar_type(), c_type, [&] {
    uint32_t vec_size = 16 / sizeof(c_type);
    dim3 block(std::min(d / vec_size, 1024U));
    tf_hip::activation::act_only_kernel<c_type, gelu_quick_act>
        <<<grid, block, 0, stream>>>(static_cast<c_type*>(out.data_ptr()), static_cast<c_type*>(input.data_ptr()), d);
