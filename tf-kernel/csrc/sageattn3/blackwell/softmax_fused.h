...
struct SoftmaxFused {
  ...
  CUTLASS_DEVICE auto online_softmax_with_quant(TensorAcc& acc, TensorMax& AbsMaxP, const float softmax_scale_log2) {
    ...
    if constexpr (FirstTile) {
      ...
      for (int mi = 0; mi < size<0>(acc_reduction_view); mi++) {
        ...
        for (int ni = 0; ni < size<1, 1>(acc_reduction_view); ni++) {
          ...
          float max_recv =
              __shfl_xor_sync(int32_t(-1), AbsMaxP(mi, ni), 1);  // exchange max with neighbour thread of 8 elements
        ...
        const float max_scaled =
            InfCheck ? (row_max(mi) == -INFINITY ? 0.f : (row_max(mi) * softmax_scale_log2 + fp8_scalexfp4_scale_log2))
      ...
      for (int mi = 0; mi < size<0>(acc_reduction_view); mi++) {
        ...
        for (int ni = 0; ni < size<1, 1>(acc_reduction_view); ni++) {
          ...
          float max_recv =
              __shfl_xor_sync(int32_t(-1), local_max, 1);  // exchange max with neighbour thread of 8 elements
        ...
        float scores_max_cur = !InfCheck ? row_max(mi) : (row_max(mi) == -INFINITY ? 0.0f : row_max(mi));
