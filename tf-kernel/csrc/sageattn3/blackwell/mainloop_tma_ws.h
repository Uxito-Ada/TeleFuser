...
struct CollectiveMainloopFwd {
  ...
  static Params to_underlying_arguments(Arguments const& args) {
    ...
    TMA_Q tma_load_Q =
        make_tma_copy(GmemTiledCopy{}, mQ, SmemLayoutQ{}, select<0, 2>(TileShape_MNK{}), _1{});  // no mcast for Q
    ...
    LayoutDS layout_ds =
        tile_to_shape(SmemLayoutAtomDS{}, make_shape(Seqlen_Q, Seqlen_K, HeadNum, Batch), Step<_2, _1, _3, _4>{});
  ...
  CUTE_HOST_DEVICE constexpr auto get_layoutSFA_TV(TiledMma& mma) {
    ...
    auto atile = make_tile(
        ...
        make_tile(
            make_layout(
                make_shape(size<1>(thr_layout_vmnk), size<2>(thr_layout_vmnk)), make_stride(Int<1>{}, Int<0>{})),
            _));
  ...
  CUTE_HOST_DEVICE constexpr auto get_layoutSFB_TV(TiledMma& mma) {
    ...
    auto btile = make_tile(
        ...
        make_tile(
            make_layout(
                make_shape(size<1>(thr_layout_vmnk), size<2>(thr_layout_vmnk)), make_stride(Int<0>{}, Int<1>{})),
            _));
  ...
  CUTLASS_DEVICE void load(
    ...
    Tensor gSFVt = local_tile(
        mSFVt(_, _, bidh, bidb), make_shape(shape<2>(TileShape_MNK{}), shape<1>(TileShape_MNK{})), make_coord(_0{}, _));
    ...
    if (lane_predicate) {
      ...
      copy(
          mainloop_params.tma_load_K.with(*pipeline_k.producer_get_barrier(smem_pipe_write_k), mcast_mask_kv),
          tKgK(_, n_block),
          tKsK(_, smem_pipe_write_k.index()));
      copy(
          mainloop_params.tma_load_SFK.with(*pipeline_k.producer_get_barrier(smem_pipe_write_k), mcast_mask_kv),
          tKgSFK(_, n_block),
          tKsSFK(_, smem_pipe_write_k.index()));
      copy(
          mainloop_params.tma_load_DS.with(*pipeline_k.producer_get_barrier(smem_pipe_write_k), mcast_mask_kv),
          tDSgDS(_, n_block),
          tDSsDS(_, smem_pipe_write_k.index()));
      ++smem_pipe_write_k;
      pipeline_v.producer_acquire(smem_pipe_write_v);
      copy(
          mainloop_params.tma_load_Vt.with(*pipeline_v.producer_get_barrier(smem_pipe_write_v), mcast_mask_kv),
          tVgVt(_, n_block),
          tVsVt(_, smem_pipe_write_v.index()));
      copy(
          mainloop_params.tma_load_SFVt.with(*pipeline_v.producer_get_barrier(smem_pipe_write_v), mcast_mask_kv),
          tVgSFVt(_, n_block),
...
#pragma unroll 2
  ...
  mma(Params const& mainloop_params,
    ...
    auto smem_tiled_copy_SFQ = make_tiled_copy_impl(
        SmemCopyAtomSF{}, get_layoutSFA_TV(tiled_mma_qk), make_shape(size<0>(tile_shape_mnk), size<2>(tile_shape_mnk)));
    ...
    auto smem_tiled_copy_SFK = make_tiled_copy_impl(
        SmemCopyAtomSF{}, get_layoutSFB_TV(tiled_mma_qk), make_shape(size<1>(tile_shape_mnk), size<2>(tile_shape_mnk)));
    ...
    auto smem_tiled_copy_SFV = make_tiled_copy_impl(
        SmemCopyAtomSF{}, get_layoutSFB_TV(tiled_mma_pv), make_shape(size<1>(tile_shape_mnk), size<2>(tile_shape_mnk)));
    ...
    //     cute::gemm(tiled_mma_qk, make_zip_tensor(tSrQ(_, _, block_id), tSrSFQ(_, _, block_id)),
    //     make_zip_tensor(tSrK(_, _, block_id), tSrSFK(_, _, block_id)), tSrS);
    // };
    // auto gemm_pv = [&](auto block_id) {
    //     cute::gemm(tiled_mma_pv, make_zip_tensor(tOrP(_, _, block_id), tOrSFP(_, _, block_id)),
    //     make_zip_tensor(tOrVt(_, _, block_id), tOrSFVt(_, _, block_id)), tOrO);
    // };
    ...
    auto quantize = [&](auto mma_k, auto acc_conversion_view) {
      ...
      for (int i = 0; i < size(AbsMaxP_stagek); i += 4) {
        ...
        flash::packed_float_to_ue4m3(
            AbsMaxP_stagek(i), AbsMaxP_stagek(i + 1), AbsMaxP_stagek(i + 2), AbsMaxP_stagek(i + 3), tmp);
      }
