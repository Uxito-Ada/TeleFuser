...
void run_mha_fwd(Flash_fwd_params& params, cudaStream_t stream, bool force_split_kernel = false) {
  BOOL_SWITCH(params.is_bf16, IsBF16, ([&] { run_mha_fwd_dispatch_dtype<IsBF16>(params, stream); }));
}

at::Tensor sageattn3_fp4_attn(
    at::Tensor& q,        // batch_size x seqlen_q x num_heads x (head_size // 2)
    ...
    const at::Tensor& delta_s,
    int64_t unpadded_k,
    at::Tensor& out,  // batch_size x num_heads x seqlen_q x head_size (output tensor)
    const double softmax_scale,
    bool is_causal,
    bool per_block_mean,
    bool is_bf16,
    int64_t return_lse  // if > 0, return softmax_lse tensor
) {
  ...
  TORCH_CHECK(
      unpacked_head_size == 64 || unpacked_head_size == 128 || unpacked_head_size == 256,
  ...
  auto out_dtype = is_bf16 ? at::ScalarType::BFloat16 : at::ScalarType::Half;
  TORCH_CHECK(out.dtype() == out_dtype, "Output tensor dtype must match is_bf16 setting");
  TORCH_CHECK(
      out.sizes() == torch::IntArrayRef({batch_size, num_heads, seqlen_q, unpacked_head_size}),
      "Output tensor must have shape (batch_size, num_heads, seqlen_q, head_size)");
  CHECK_DEVICE(out);
  ...
  set_params_fprop(
      ...
      /*seqused_k=*/nullptr,
      nullptr,
      return_lse > 0 ? softmax_lse.data_ptr() : nullptr,
      /*p_dropout=*/0.f,
      softmax_scale,
      /*window_size_left=*/-1,
  ...
  auto tile_count_semaphore =
      is_causal ? torch::full({1}, 132, opts.dtype(torch::kInt32)) : torch::empty({1}, opts.dtype(torch::kInt32));
