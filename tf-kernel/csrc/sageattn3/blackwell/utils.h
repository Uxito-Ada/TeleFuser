...
#pragma unroll
  for (int mi = 0; mi < size<0>(tensor); ++mi) {
    ...
    const float max_scaled = Check_inf ? (max(mi) == -INFINITY ? 0.f : (max(mi) * (Scale_max ? scale : float(M_LOG2E))))
...
CUTLASS_DEVICE void
packed_float_to_ue4m3(float const& f0, float const& f1, float const& f2, float const& f3, uint32_t& out) {
  asm volatile(
      "{\n"
      ".reg .b16 lo;\n"
      ".reg .b16 hi;\n"
...
CUTLASS_DEVICE void add(float2& c, float2 const& a, float2 const& b) {
...
CUTLASS_DEVICE void add_inplace(float2& a, float2 const& b) {
...
CUTLASS_DEVICE void sub(float2& c, float2 const& a, float2 const& b) {
  asm volatile("sub.f32x2 %0, %1, %2;\n"
               : "=l"(reinterpret_cast<uint64_t&>(c))
               : "l"(reinterpret_cast<uint64_t const&>(a)), "l"(reinterpret_cast<uint64_t const&>(b)));
}

CUTLASS_DEVICE void sub_inplace(float2& a, float2 const& b) {
...
CUTLASS_DEVICE void mul(float2& c, float2 const& a, float2 const& b) {
  asm volatile("mul.f32x2 %0, %1, %2;\n"
               : "=l"(reinterpret_cast<uint64_t&>(c))
               : "l"(reinterpret_cast<uint64_t const&>(a)), "l"(reinterpret_cast<uint64_t const&>(b)));
}

CUTLASS_DEVICE void fma(float2& d, float2 const& a, float2 const& b, float2 const& c) {
...
CUTLASS_DEVICE void fma_inplace(float2& a, float2 const& b, float2 const& c) {
  asm volatile("fma.rn.f32x2 %0, %0, %1, %2;\n"
               : "+l"(reinterpret_cast<uint64_t&>(a))
               : "l"(reinterpret_cast<uint64_t const&>(b)), "l"(reinterpret_cast<uint64_t const&>(c)));
}
...
CUTLASS_DEVICE constexpr auto convert_to_reduction_tensor(Tensor mma_tensor) {
...
CUTLASS_DEVICE constexpr auto convert_to_conversion_layout(Layout mma_layout) {
  ...
  return make_layout(
      ...
      layout<1>(mma_layout),
      layout<1>(mma_n_division));
}

template <class Tensor>
CUTLASS_DEVICE constexpr auto convert_to_conversion_tensor(Tensor mma_tensor) {
